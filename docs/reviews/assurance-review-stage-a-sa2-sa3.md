...
- [claude | research/2026-08-23-subscriber-stage-b.md increment 4 (i), docs/stage-b1-proof-map.md §2.4 | overwatch T7] APPLIED — the relation's split rule "visited vs unvisited" → "past its linearization point (check for an overflow decision, resolve for an admit) vs not"; kernel-checked counter-instance `RtTraces.caseBetween` (`research/logs/rt_probe10.lean`). Not a frozen statement; the method only.
