...
| `SubCore.lean` | lookups through `updateStream`/`removeStream`/`insertStream`; `publishStep_ok_eq`, `deleteStep_ok_eq`, `createStep_lookup_preserved`, `step_lookup_preserved`; `entrySequences_*` list helpers; `pairwise_*`; `SubInv.of_stream_lookup` | facts about the first-slice core the subscriber layer consumes |
| `SubProofs.lean` | `*_core` frame lemmas, SA1 `reachableSub_core`; `selectReplay_sublist/_mem/_pairwise`; per-label preservation `newSubscriber_inv`, `pullStep_inv`, `unsubscribe_inv`, `deliverOne_inv`, `endOne_inv`; `SubInv.core_eq`, `SubInv.of_lookups` | one preservation lemma per transition function |
| `SubReachable.lean` | `mem_of_lookupSub`, `mem_updateSub`; `afterOp_inv`, `applyOp_inv`, `applyRegister_inv`, `applyPull_inv`, `applyUnsubscribe_inv`, `apply_inv`; SA2 `stateInv_reachable`, SA3 `pending_le_capacity`; **`reachableSub_all`**; `lookupSub_*` (update/fresh/append/map/of_mem_pairwise), `updateSub_keys`, `keys_map_snd`, `applyRegister_enabled`, `shape_of_keys`, `apply_shape`, `subShape_reachable`, `lookupSub_nextId` | the only `ReachableSub` induction and the principle everything else uses |
| `SubStatements.lean` | `sub_core_inv`; `publishedMessage`; `applyOp_ok_eq`, `afterOp_publish_sub`, `publish_sub`; the three faces `deliverOne_admit`/`_overflow`/`_skip`, `endOne_skip`; SA4a/c/d; SA5; SA6; `LagInv`, `LagState`, `lagInv_*`, `lagState_afterOp`, `apply_lag`, `lagState_reachable`; SA7; `applyAll`, `Negative`, `subNegatives`, `all_sub_negatives` | the frozen statements beyond SA1–SA3 |
| `SubHistory.lean` | `RegInfo`, `SubStateH`, `initialSubH`, `erase`, `lookupReg`, `labelMessage`, `committedAfter`, `regsAfter`, `applyH`, `NextH`, `ReachableSubH`, `liveKeep`, `liveOf`, `liveEntries`; AV1/AV2; `lookupReg_*`, `liveOf_*`; `lookupStream_removeStream_self`, `createStep_ok_shape`, `getStep_ok_eq`, `lastMsgStep_ok_eq`, `mem_updateSub_eq`; `HistInv`, `histInv_*`, `histInv_reachable`; SA5h `visible_global`, `entries_committed` | the proof-only ledger (history variable) |
